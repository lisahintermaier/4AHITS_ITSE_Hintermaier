...
EOF
```
Was macht das Skript: nc 192.168.18.51 80 << EOF ... EOF öffnet mit netcat eine TCP-Verbindung zu 192.168.18.51 Port 80 und sendet den zwischen EOF stehenden Text als Anfrage.
Die Anfrage ist ein HTTP HEAD / (fordert nur die Header an) mit Host und Connection: close — der Server schickt nur die HTTP-Header zurück und schließt danach die Verbindung.   
...
<td>
        <code>telnet 192.168.18.51 80</code> → dann <code>HEAD / HTTP/1.0</code> + <code>Host: 192.168.18.51</code> + 2 leere Zeilen
</td>
<td>Einfach; manuelles Senden von Text nötig. Nicht verschlüsselt.</td>
</tr>
 
<tr>
<td><code>nc</code> (netcat)</td>
<td>TCP, Scripting, Banner-Grabbing</td>
<td>
        Port-Check (verbindungsprobe): <code>nc -vz 192.168.18.51 22</code><br>
        Timeout setzen: <code>nc -vz -w 3 192.168.18.51 22</code><br>
        HTTP Header:<br>
<code>printf 'HEAD / HTTP/1.0\r\nHost: 192.168.18.51\r\nConnection: close\r\n\r\n' | nc 192.168.18.51 80</code>
</td>
<td>Sehr flexibel — gut für Skripte. <code>-w</code> für Timeout; <code>-z</code> nur „scan“ (no I/O).</td>
</tr>
...
<td>
        Kurz Header: <code>curl -sI http://192.168.18.51/</code><br>
        Verbose / Debug: <code>curl -v http://192.168.18.51/</code><br>
...
### Aufgaben:   
### 1.Unteraufgabe: Führe einen port scan auf die Metasploitable VM durch. Analysiere das Ergebnis.   
### Lösung:   
Befehl: `nmap 192.168.18.51` 
...
### 2. Unteraufgabe: Experimentiere mit den Optionen für (Victim ist Metasploitable VM):   
### 2.1 :Scannen aller möglichen Ports. Macht das bei Metasploitable, verglichen zum Default Verhalten, einen Unterschied?
### Lösung:  
Befehl: `nmap -p- 192.168.18.51` 
...
Auf Metasploitable kann das zusätzliche Dienste auf hohen Ports bringen, die der Default-Scan ausgelassen hätte. Metasploitable enthält oft Dienste auf unüblichen Ports. Für vollständige Erfassung ist ein Full-port-scan sinnvoll.

### weitere Übungen sind sich Zeitlich nicht mehr ausgegangen.
